...
pql <- lapply(pql_blocks, read_immunaid)
...
# Metada
pql_codes <- lapply(pql_blocks, function(i) read_immunaid(i, "codes"))
...
```{r}
block <- blocks[10]
sf36 <- read_immunaid(block)
sapply(sf36, dim)
sf36_tot <- Reduce(rbind, sf36)
sf36_code <- read_immunaid(block, "codes")
...
```{r}
block <- blocks[5]
haq <- read_immunaid(block)
sapply(haq, dim)
haq_tot <- Reduce(rbind, haq)
haq_code <- read_immunaid(block, "codes")
...
```{r}
block <- blocks[2]
chaq <- read_immunaid(block)
sapply(chaq, dim)
chaq_tot <- Reduce(rbind, chaq)
chaq_code <- read_immunaid(block, "codes")
...
```{r}
block <- blocks[1]
behcet <- read_immunaid(block)
sapply(behcet, dim)
behcet_tot <- Reduce(rbind, behcet)
behcet_code <- read_immunaid(block, "codes")
...
```{r}
block <- blocks[4]
diet <- read_immunaid(block)
sapply(diet, dim)
diet_tot <- Reduce(rbind, diet)
diet_code <- read_immunaid(block, "codes")
