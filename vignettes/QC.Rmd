...
get_qc <- function(x, batch = 4, tool = "qc", geom = "line", draw = TRUE) {
  cwd <- file.path(path, tool)
...
# Phred
get_qc(4, batch, tool)
stats[[batch]][[3]] <- apply(qc, 1, median)
...
if (batch == 4) {
  ...
  qc <- rbind(r1, qc)
  # plot_gc(qc)
  stats[[batch]][[4]] <- apply(qc, 1, median)
...
# kallisto
# get_qc(1, batch, geom = "bar")
# qc0 <- read_qc_summary(batch, "kallisto")
# qc0 <- filter(qc0, `% aligned` > 0)
# stats[[batch]][[6]] <- pull(qc0, `% aligned`)
...
titles <- c("Adaptaters %", "Unknown bases %", "Phred median", paste0("Duplicates % (", c("Before", "After"), ")"))
col <- c(1, 2, 3, 5, 4)
col_name <- c(lapply(seq(3), function(x) seq(4)[-2]), lapply(seq(2), function(x) seq(4)))
p <- list.map(
  c(1, 2, 3, 5, 7),
  f(i, j) ~ list.map(stats, pluck(., i)) %>% compact() %>% list_cbind() %>% set_colnames(col_name[[j]]) %>%  plot_violin0(colour = rep(get_colors()[col[j]], ncol(.)), title = titles[j], pch_colour = "gray20", pch_alpha = 0.5, stat = FALSE, wrap = 10)
)
```
