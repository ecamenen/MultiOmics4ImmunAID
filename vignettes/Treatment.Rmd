                ...
                list.map({
                    ...
                    if (type == 2) {
                        for (i in c("il", "cd", "tnf")) {
                            res <- str_replace_all(res, paste0("Anti[- ](", i, ") ?(\\d{1,2}|α)$"), paste0("Anti-", toupper(i), "\\2"))
                        }
                    } else if (type == 3) {
                        res <- str_replace_all(res, "Antibiot(ic|ique|herapie)", "Anti-biotic") %>%
...
# Data loading
```{r setup}
clinic <- read_immunaid(path = data_path) %>%
    Reduce(rbind, .) %>%
    formate_immunaid()
clinic_code <- read_immunaid(, "codes", path = data_path)
...
write.xlsx(list_treat2, file = file.path(data_path, "still_treatments.xlsx"))
```
