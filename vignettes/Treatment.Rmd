...
treat_types <- list_treat3$`Ongoing at V1.Type` %>%
    unlist() %>%
    sort() %>%
    unique()
t_treat_types <- list.map(treat_types, f(i) ~ list.mapv(unique(list_treat3$`Patient.ID`), f(j) ~ i %in%  (filter(list_treat3, `Patient.ID` == j) %>% pull("Ongoing at V1.Type"))) %>% as.numeric()) %>% list.cbind() %>%
  set_rownames(ids)
apply(t_treat_types, 2, sum)

treat_subtypes <- list_treat3$`Ongoing at V1.Subtype` %>%
    unlist() %>%
    sort() %>%
    unique()
t_treat_subtypes <- list.map(treat_subtypes, f(i) ~ list.mapv(unique(list_treat3$`Patient.ID`), f(j) ~ i %in% (filter(list_treat3, `Patient.ID` == j) %>% pull("Ongoing at V1.Subtype"))) %>% as.numeric()) %>% list.cbind() %>%
  set_rownames(ids)
apply(t_treat_subtypes, 2, sum)

l_drugs <- list_treat3$`Ongoing at V1.Drug name` %>%
    unlist() %>%
    unique() %>%
    na.omit()
t_drugs <- list.map(
    unique(list_treat3$`Patient.ID`), f(j) ~ {
        list.mapv(l_drugs, f(k) ~ (k %in% (filter(list_treat3, `Patient.ID` == j) %>% pull("Ongoing at V1.Drug name"))))
    } %>% as.numeric()
) %>%
    list.rbind() %>%
...
l_per_treat <- list.map(treat_types, f(i) ~ {
    l_drugs <- filter(list_treat3, `Ongoing at V1.Type` == i) %>% pull(`Ongoing at V1.Drug name`) %>%
        unlist() %>%
        unique() %>%
        na.omit()
    list.map(
        unique(list_treat3$`Patient.ID`), f(j) ~ {
            list.mapv(l_drugs, f(k) ~ (k %in% (filter(list_treat3, `Patient.ID` == j) %>% filter(`Ongoing at V1.Type` == i) %>% pull(`Ongoing at V1.Drug name`))))
        } %>% as.numeric()
    ) %>%
        list.rbind() %>%
        set_colnames(l_drugs)
...
l_per_bio_treat <- list.map(treat_subtypes, f(i) ~ {
    l_drugs <- filter(list_treat3, `Ongoing at V1.Subtype` == i) %>% pull(`Ongoing at V1.Drug name`) %>%
        unlist() %>%
        unique() %>% 
        na.omit()
    list.map(unique(list_treat3$`Patient.ID`), f(j) ~ {
        list.mapv(l_drugs, f(k) ~ (k %in% (filter(list_treat3, `Patient.ID` == j) %>% filter(`Ongoing at V1.Subtype` == i) %>% pull(`Ongoing at V1.Drug name`))))
    } %>% as.numeric()) %>%
        list.rbind() %>%
        set_colnames(l_drugs)
}) %>% compact()
...
disease_per_drug <- function(x, var = "Type", l = list_treat3) {
  var0 <- paste("Ongoing at V1", var, sep = ".")
  list.map(
      x,
