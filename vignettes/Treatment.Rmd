...
n_clin <- pull(clinic, C_2643_3798) %>% fct_count() %>% arrange(desc(n)) %>% rename(n2 = n)

list.map(
  treat_types,
   f(x) ~ filter(list_treat1, `Ongoing at V1.Type` == x) %>% 
...
list.map(
    treat_subtypes,
    f(x) ~ filter(list_treat1, `Ongoing at V1.Subtype` == x) %>% 
        filter(!pull(., `Patient.ID`) %>% duplicated()) %>%
        pull("Patient.Disease") %>%
        fct_count() %>% 
        left_join(n_clin) %>%
...
list.map(
    apply(t_drugs, 2, sum) %>% .[. > 1] %>% names() %>% sort(),
    f(x) ~ filter(list_treat1, `Ongoing at V1.Drug name` == x) %>% 
        filter(!pull(., `Patient.ID`) %>% duplicated()) %>%
        pull("Patient.Disease") %>%
        fct_count() %>% 
        left_join(n_clin) %>%
...
list.map(
  pull(list_treat1, `Patient.Disease`) %>% unique(),
   f(x) ~ {
     tmp <- filter(list_treat1, `Patient.Disease` == x)
     list.map(
        pull(tmp, `Patient.ID`) %>% unique(),
            f(i) ~ filter(tmp, `Patient.ID` == i) %>% filter(!pull(., `Ongoing at V1.Type`) %>% duplicated())
    ) %>%
        list.rbind() %>% 
       pull("Ongoing at V1.Type") %>%
       fct_count() %>% 
...
list.map(
  pull(list_treat1, `Patient.Disease`) %>% unique(),
   f(x) ~ {
     ...
     list.map(
        pull(tmp, `Patient.ID`) %>% unique(),
            f(i) ~ filter(tmp, `Patient.ID` == i) %>% filter(!pull(., `Ongoing at V1.Drug name`) %>% duplicated())
    ) %>%
        list.rbind() %>% 
