...
list_treat2 <- select(list_treat1, -ID2) %>%
    set_colnames(
        colnames(.) %>%
...
treat_types <- list.map(list_treat, f(i) ~ list.map(i, f(j) ~ list.map(j, f(k) ~ k[names(k) == "Type"]))) %>%
    ...
    sort() %>%
    unique()
t_treat_types <- list.map(treat_types, f(i) ~ list.mapv(unique(list_treat1$ID2), f(j) ~ i %in%  (filter(list_treat1, ID2 == j) %>% pull("Ongoing at V1.Type"))) %>% as.numeric()) %>% list.cbind()
...
treat_subtypes <- list.map(list_treat, f(i) ~ list.map(i, f(j) ~ list.map(j, f(k) ~ k[names(k) == "Subtype"]))) %>%
    ...
    sort() %>%
    unique()
t_treat_subtypes <- list.map(treat_subtypes, f(i) ~ list.mapv(unique(list_treat1$ID2), f(j) ~ i %in% (filter(list_treat1, ID2 == j) %>% pull("Ongoing at V1.Subtype"))) %>% as.numeric()) %>% list.cbind()
...
l_drugs <- list_treat1$`Ongoing at V1.Drug name` %>%
    ...
    na.omit()
t_drugs <- list.map(
    unique(list_treat1$ID2), f(j) ~ {
        list.mapv(l_drugs, f(k) ~ (k %in% (filter(list_treat1, ID2 == j) %>% pull("Ongoing at V1.Drug name"))))
    } %>% as.numeric()
) %>%
    list.rbind() %>%
...
l_per_treat <- list.map(treat_types, f(i) ~ {
    ...
    list.map(
        unique(list_treat1$ID2), f(j) ~ {
            list.mapv(l_drugs, f(k) ~ (k %in% (filter(list_treat1, ID2 == j) %>% filter(`Ongoing at V1.Type` == i) %>% pull(`Ongoing at V1.Drug name`))))
        } %>% as.numeric()
    ) %>%
        list.rbind() %>%
        set_colnames(l_drugs)
...
l_per_bio_treat <- list.map(treat_subtypes, f(i) ~ {
    l_drugs <- filter(list_treat1, `Ongoing at V1.Subtype` == i) %>% pull(`Ongoing at V1.Drug name`) %>%
        ...
        unique() %>% 
        na.omit()
    list.map(unique(list_treat1$ID2), f(j) ~ {
        list.mapv(l_drugs, f(k) ~ (k %in% (filter(list_treat1, ID2 == j) %>% filter(`Ongoing at V1.Subtype` == i) %>% pull(`Ongoing at V1.Drug name`))))
    } %>% as.numeric()) %>%
        list.rbind() %>%
        set_colnames(l_drugs)
}) %>% compact()
...
cols1 <- get_colors1()[seq(ncol(t_treat_types))] %>% rev()
cols2 <- sapply(names(l_per_treat0), function(x) filter(tmp, ID == x) %>% pull(rank))
cols3 <- cols4[which(names(cols4) == "Aspirin")] %>% get_colors1()[.]
tmp <- cols4[which(names(cols4) == "Biologic")] %>% get_colors1()[.]
l_per_bio_treat0 <- l_per_bio_treat %>% purrr:::discard(str_detect(names(.), "inflammatory|aggregation"))
greens <- colorRampPalette(c("white", tmp, "black"))(length(l_per_bio_treat0) + 4)[-c(1:2, length(l_per_bio_treat0) + 3:5)]
cols <- c(1, get_colors1()[cols2], greens, tmp, cols3)
p_treat <- c(list(`Type of treatment` = t_treat_types), l_per_treat0, l_per_bio_treat0, list(Biologic = select(as.data.frame(t_treat_subtypes), -contains(c("inflammatory", "aggregation"))), Aspirin = select(as.data.frame(t_treat_subtypes), contains(c("inflammatory", "aggregation")))))
