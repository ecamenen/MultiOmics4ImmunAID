...
t_treat_response <- table_drug_response("Type") %>% 
  cbind(table_drug_response("Subtype"))  %>%
    ...
    mutate(ID = paste0(as.numeric(ID))) %>% 
    pivot_longer(cols = -ID, names_to = "Treatment", values_to = "Response")

t_treat_types0 <- t_treat_types %>%
    as.data.frame() %>%
    select(-Biologic) %>%
    mutate(ID = paste0(as.numeric(ID))) %>% 
    pivot_longer(cols = -ID, names_to = "Treatment", values_to = "Values")

cols <- sort(colnames(t_treat_types0)) %>% data.frame(ID = .) %>% left_join(data.frame(ID = names(cols4), val = cols4)) %>% pull(val)
