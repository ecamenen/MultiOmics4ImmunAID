                ...
                list.map({
                    ...
                    if (type %in% c(2, 3)) {
                        ...
                        res <- str_replace_all(res, "Antibiot(ic|ique|herap((ie)|y))", "Anti-biotic") %>%
                            ...
                            str_replace_all("Kineret", "Anakinra") %>%
...
# Kineret
if (!is.null(list_treat[["9005"]])) {
    list_treat[["9005"]][["5"]]$Type <- "Biologic"
    list_treat[["9005"]][["5"]]$Subtype <- "Anti-IL1"
}

if (!is.null(list_treat[["7006"]])) {
    list_treat[["7006"]][["3"]]$Type <- "Biologic"
    list_treat[["7006"]][["3"]]$Subtype <- "Anti-IL1"
    # Xeljanz
    list_treat[["7006"]][["4"]]$Type <- "Kinase inhibitors"
}
...
list_treat <- list.map(list_treat, f(i) ~ { list.class(i, f(j) ~ {
  status <- NULL

  if (is.list(j)) {

    if (j[["Discontinuation"]] == "Yes" && !is.null(j[["Date of discontinuation"]])) {
        if (dmy(j[["Date of discontinuation"]]) < dmy(i[["Date of the 1st visit"]])) {
            status <- c(status, "Pre-inclusion")
        }
    }

    if (dmy(j[["Date of beginning"]]) < dmy(i[["Date of the 1st visit"]])) {
        if (j[["Discontinuation"]] == "No" || is.null(j[["Date of discontinuation"]])) {
            status <- c(status, "Ongoing at V1")
        } else {
            if (dmy(j[["Date of discontinuation"]]) >= dmy(i[["Date of the 1st visit"]])) {
                status <- c(status, "Ongoing at V1")
            }
    ...
    if (i[["Eligible for a 2nd visit"]] == "Yes") {
      if (i[["Date of the 2nd visit"]] != "NA") {
          if (dmy(j[["Date of beginning"]]) < dmy(i[["Date of the 2nd visit"]])) {
              if (j[["Discontinuation"]] == "No" || is.null(j[["Date of discontinuation"]])) {
                  status <- c(status, "Ongoing at V2")
              } else {
                  if (dmy(j[["Date of discontinuation"]]) >= dmy(i[["Date of the 2nd visit"]])) {
                      status <- c(status, "Ongoing at V2")
                  } else {
                    status <- c(status, "After V1 and before V2")
                  }
              }
          } else {
              status <- c(status, "Post V2")
          }
      } else {
          if (dmy(j[["Date of beginning"]]) >= dmy(i[["Date of the 1st visit"]])) {
              status <- c(status, "Post V2")
    ...
    status <- c(status, "Patient")
}
        if (length(status) > 1) {
      print(c(">1", i$ID, i[["Eligible for a 2nd visit"]], i[["Date of the 1st visit"]], i[["Date of the 2nd visit"]], j))
        }
  
    # Check if any cases are forgotten
    if (is.null(status)) {
      print(c("NULL", i$ID, i[["Eligible for a 2nd visit"]], i[["Date of the 1st visit"]], i[["Date of the 2nd visit"]], j))
    }
...
var_names2 <- c("Date of beginning", "Type", "Subtype", "Drug name", paste(c("Initial", "Maximal", "Administrated"), "dose"), "Response", "Discontinuation", "Date of discontinuation", rep("Reason for discont.", 4))
l_var_names <- list.map(
    c("Pre-inclusion", paste("Ongoing at", c("V1", "V2")), "Post V2", "After V1 and before V2"),
...
list_treat1 <- list.map(
    ...
    as_tibble() %>%
    select(., contains(c(var_names[c(1, 5)], l_var_names[[1]], "N previous treatment", var_names[2], l_var_names[[2]], var_names[c(3, 6)], l_var_names[[3]], var_names[4], l_var_names[[4]], l_var_names[[5]])))
...
if (!is.null(list_treat1[["Post V2.Reason for discont. (side effect(s))"]])) {
    list_treat1 <- relocate(list_treat1, "Post V2.Reason for discont. (side effect(s))", .after = "After V1 and before V2.Date of beginning")
}

if (!is.null(list_treat1[["After V1 and before V2.Reason for discont. (side effect(s))"]])) {
    list_treat1 <- relocate(list_treat1, "After V1 and before V2.Reason for discont. (side effect(s))", .after = last_col())
}
...
list_treat2 <- list_treat1 %>% set_colnames(colnames(.) %>%
            str_remove_all("((Patient)|(Ongoing at ((V1)|(V2)))|(Pre-inclusion)|(Post V2)|(After V1 and before V2)).")
    )
...
ids <- unique(list_treat3$Patient.ID)
...
treat_types <-  pull(list_treat3, paste0("Ongoing at ", visit,".Type")) %>%
    unlist() %>%
    sort() %>%
    unique()
t_treat_types <- list.map(treat_types, f(i) ~ list.mapv(unique(list_treat3$`Patient.ID`), f(j) ~ i %in%  (filter(list_treat3, `Patient.ID` == j) %>% pull(paste0("Ongoing at ", visit,".Type")))) %>% as.numeric()) %>% list.cbind() %>%
  set_rownames(ids)
apply(t_treat_types, 2, sum)

treat_subtypes <- pull(list_treat3, paste0("Ongoing at ", visit,".Subtype")) %>%
    unlist() %>%
    sort() %>%
    unique()
t_treat_subtypes <- list.map(treat_subtypes, f(i) ~ list.mapv(unique(list_treat3$`Patient.ID`), f(j) ~ i %in% (filter(list_treat3, `Patient.ID` == j) %>% pull(paste0("Ongoing at ", visit,".Subtype")))) %>% as.numeric()) %>% list.cbind() %>%
  set_rownames(ids)
apply(t_treat_subtypes, 2, sum)

l_drugs <- pull(list_treat3, paste0("Ongoing at ", visit,".Drug name")) %>%
    unlist() %>%
    unique() %>%
    na.omit()
t_drugs <- list.map(
    unique(list_treat3$`Patient.ID`), f(j) ~ {
        list.mapv(l_drugs, f(k) ~ (k %in% (filter(list_treat3, `Patient.ID` == j) %>% pull(paste0("Ongoing at ", visit,".Drug name")))))
    } %>% as.numeric()
) %>%
    list.rbind() %>%
    set_colnames(l_drugs)

get_per_treat <- function(var = "Type", time = V1, l = list_treat3) {
    if (var == "Type") {
      l0 <- treat_types
    } else {
      l0 <- treat_subtypes
    ...
    list.map(l0, f(i) ~ {
      l_drugs <- filter(l, !!sym(paste0("Ongoing at ", visit,".", var)) == i) %>% pull(paste0("Ongoing at ", visit,".Drug name")) %>%
          unlist() %>%
          unique() %>%
          na.omit()
      list.map(
          unique(l$`Patient.ID`), f(j) ~ {
              list.mapv(l_drugs, f(k) ~ (k %in% (filter(l, `Patient.ID` == j) %>% filter(!!sym(paste0("Ongoing at ", visit,".", var)) == i) %>% pull(paste0("Ongoing at ", visit,".Drug name")))))
          } %>% as.numeric()
      ) %>%
          list.rbind() %>%
          set_colnames(l_drugs)
...
l_per_treat <- get_per_treat("Type", visit)
list.map(l_per_treat, f(i) ~ apply(i, 2, sum))

l_per_bio_treat <- get_per_treat("Subtype", visit)
...
l_biol <- select(as.data.frame(t_treat_subtypes), -contains(c("inflammatory", "aggregation"))) %>% remove_constant()
tmp <- apply(l_biol, 2, sum) %>% sort(TRUE) %>% data.frame(ID = names(.), val = ., rank = seq_along(.))
cols5 <- sapply(colnames(l_biol), function(x) filter(tmp, ID == x) %>% pull(rank)) %>% rev()
greens <- colorRampPalette(c("white", get_colors()[3], "black"))(ncol(l_biol) + 4)[-c(1:2, ncol(l_biol) + 3:5)]
cols <- c(get_colors1()[cols2], greens[cols5], get_colors()[3], cols3)
p_treat <- c(l_per_treat0, l_per_bio_treat %>% purrr:::discard(str_detect(names(.), "inflammatory|aggregation")), list(Biologic = l_biol, Aspirin = select(as.data.frame(t_treat_subtypes), contains(c("inflammatory", "aggregation")))))
    ...
    f(i, j, k) ~ {
      if (ncol(i) > 0) {
        if (j == (length(.) - 1)) {
            ...
            colors <- colorRampPalette(c("gray", cols[j]))(ncol(i))
        }
        as.data.frame(i) %>%
            mutate(temp = 0) %>%
            plot_bar_mcat(
...
# Stats per disease
```{r, eval = FALSE}
n_clin <- pull(clinic, C_2643_3798) %>% 
  fct_count() %>% 
  arrange(desc(n)) %>% 
  rename(n2 = n)

disease_per_drug <- function(x, var = "Type", l = list_treat3) {
  var0 <- paste0("Ongoing at ", visit, ".", var)
  list.map(
      x,
      f(x) ~ filter(l, !!sym(var0) == x) %>% 
          filter(!pull(., `Patient.ID`) %>% duplicated()) %>%
          pull("Patient.Disease") %>%
          fct_count() %>% 
          left_join(n_clin) %>%
          mutate(n2 = round(n / n2 * 100)) %>%
          arrange(desc(n2))
  )
}

disease_per_drug(treat_types, "Type")
disease_per_drug(treat_subtypes, "Subtype")
apply(t_drugs, 2, sum) %>%
  .[. > 1] %>%
  names() %>% 
  sort() %>%
  disease_per_drug("Drug name")

drug_per_disease <- function(var = "Type", l = list_treat3) {
  var0 <- paste0("Ongoing at ", visit, ".", var)
  list.map(
    pull(l, `Patient.Disease`) %>% unique(),
     f(x) ~ {
       tmp <- filter(l, `Patient.Disease` == x)
       list.map(
          pull(tmp, `Patient.ID`) %>% unique(),
              f(i) ~ filter(tmp, `Patient.ID` == i) %>% filter(!pull(., !!!var0) %>% duplicated())
      ) %>%
          list.rbind() %>% 
         pull(var0) %>%
         fct_count() %>% 
         mutate(n2 = round(n / (filter(n_clin, f == x) %>% pull(n2)) * 100)) %>%
         arrange(desc(n2))
     }
  )
}

drug_per_disease0 <- function(var = "Type", var2 = "n2", l = list_treat3) {
  drug_per_disease(var, l) %>%
    list.map(
      f(i, j, k) ~ {
        filter(i, !is.na(f)  & f != "Biologic") %>% 
          mutate(n = !!sym(var2)) %>%
          select(f, n) %>%
          set_colnames(c("f", k))
      }
    ) %>% Reduce(function(x, y) full_join(x, y), .) %>% 
    t() %>% row_to_names(1) %>% as.data.frame()
}
drug_per_disease("Type")
drug_per_disease("Subtype")

drug_per_disease0("Type") %>% 
  rownames_to_column() %>% 
  left_join(drug_per_disease0("Subtype") %>% rownames_to_column()) %>% 
  column_to_rownames() %>% 
  mutate(`Anti-IL6` = 0) %>%
  select(colnames(.) %>% rev()) %>%
  plot_radar(FALSE, col_alpha = 0.25)

drug_per_disease0("Type", "n") %>% 
    rownames_to_column() %>% 
    left_join(drug_per_disease0("Subtype", "n") %>% rownames_to_column()) %>% 
    column_to_rownames() %>% 
    select(colnames(.) %>% rev()) %>%
    plot_radar(FALSE, FALSE, 0, 3)
```

# Drug per patient
```{r}
parse_response <- function(x, y) {
  if (str_detect(na.omit(x), y) %>% any()) {
    x[x == y] %>% na.omit()
  } else {
    x
  }
...
table_drug_response <- function(var = "Type") {
  if (var == "Type") {
    t_var <- treat_types
  } else {
    t_var <- treat_subtypes
  }
  list.map(
      t_var,
      f(x) ~ {
          tmp <- list.map(
              list_treat4, 
              f(i) ~ {
                  if (!is.null(i[[paste0("Ongoing at ", visit)]])){
                      temp <- list.map(
                          i[[paste0("Ongoing at ", visit)]],
                          f(j) ~ {
                              if(!is.null(j[[var]]) && j[[var]] == x) {
                                  ifelse(!is.null(j$Response), j$Response, NA)
                              }
                          }
                      ) %>%
                          unlist() %>% unique()
                      parse_response(temp, "Complete response") %>% 
                        parse_response("Partial response") %>%
                        parse_response("Primary failure")
                  }
              }
          ) 
...
t_treat_response <- table_drug_response("Type") %>% 
  cbind(table_drug_response("Subtype"))  %>%
    set_colnames(colnames(.) %>% str_replace_all("(Anti).", "\\1-") %>% str_replace_all("\\.", " ")) %>%
  as.data.frame() %>%
    mutate(ID = paste0(as.numeric(row.names(.)))) %>% 
    pivot_longer(cols = -ID, names_to = "Treatment", values_to = "Response")
# write_tsv(t_treat_response, file = "table_drug_response.tsv")

t_treat_types1 <- t_treat_types %>%
    as.data.frame() %>%
    select(-Biologic)
t_treat_types0 <- t_treat_types1 %>%
    cbind(t_treat_subtypes) %>%
    mutate(ID = paste0(as.numeric(row.names(.)))) %>% 
    pivot_longer(cols = -ID, names_to = "Treatment", values_to = "Values")
# write_tsv(t_treat_types0, file = "table_drug.tsv")

cols <- sort(colnames(t_treat_types1)) %>% data.frame(ID = .) %>% left_join(data.frame(ID = names(cols4), val = cols4)) %>% pull(val)

left_join(t_treat_types0, t_treat_response, c("ID", "Treatment")) %>%
    mutate(Response = str_replace_na(Response) %>% fct_relevel(c("Complete response", "Partial response", "Loss of efficacy", "Primary failure", "NA"))) %>%
    mutate(ID = as.character(ID) %>% str_replace_all("^(\\d{4})$", "0\\1")) %>%
    ggplot(aes(x = ID, y = Values, fill = Treatment, pattern_spacing = Response, pattern = Response, pattern_density = Response, pattern_angle = Response)) +
    geom_bar_pattern(
        stat = "identity", 
        position = "stack",
        ...
        pattern_fill = "black",
        pattern_alpha = 0.5,
        pattern_aspect_ratio = 0.5
    ) +
    labs(x = "Patient", y = "") +
    theme_minimal() +
    coord_flip() +
    scale_fill_manual(values = c(greens[cols5], get_colors1()[cols])) +
    theme_perso(cex_axis = 15) +
    theme(
        legend.title = element_text(face = "bold.italic", size = 15),
        legend.text = element_text(size = 12),
        panel.grid.major.x = element_blank(),
        axis.text.x = element_blank(),
        axis.ticks.x = element_blank()
    ) + 
    scale_pattern_spacing_manual(values = seq(0.01, 0.07, length.out = 5)) +
    scale_pattern_manual(values = c("crosshatch", rep("stripe", 2), "circle", "none") %>% rev()) + 
    scale_pattern_density_manual(values = seq(0.05, 0.3, length.out = 5)) +
    scale_pattern_angle_manual(values = c(0, 45, 45, 135, 45))
```