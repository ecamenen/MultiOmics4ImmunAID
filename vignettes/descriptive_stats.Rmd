...
vars2 <- c("immun_aid_identifier", "weight_kg", "disease", "height_cm")

cl <- clinic_proc
blocks_cl <- clinic_proc %>% as.data.frame() %>% select(-c(vars2))
blocks_clinic_still <- remove_cofunding(list(CLINIC = blocks_cl), vars)

blocks_clinic_still[[1]] <- select(blocks_clinic_still[[1]], -all_of(vars))
use_data(blocks_clinic_still, overwrite = TRUE)
```
