...
# CHAQ
```{r}
chaq <- read_batches("CHAQ")
sapply(chaq, dim)
chaq_tot <- Reduce(rbind, chaq)
chaq_code <- read_batches("CHAQ", "codes")
...
chaq_tot <- mutate_all(chaq_tot, funs(str_replace_all(., paste(na_vars, collapse = ", "), "")))
...
# get_var_names(unlist(cols_sim[-i_rows]), chaq_code[[3]])
level1 <- reformat(l_level_cols[-i_rows, ])
