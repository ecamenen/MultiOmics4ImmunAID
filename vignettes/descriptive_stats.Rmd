...
sys %>% filter(str_detect(item_name, "Treatment$")) %>%
...
l_codes <- c(l_codes, (codes %>%
    slice(detect_difficulty(., "date of the visit|evolution")) %>%
    filter(!str_detect(item_name, "Persistant")))$column_code)

## 3.3 Biological values
vis2 <- codes %>% slice(detect_difficulty(., "second|followup"))
l_codes <- c(l_codes, (vis2 %>% slice(detect_difficulty(., "CRP|SAA")))$column_code)

# Extraction codes
clinic_extraction <- clinic_tot %>% select(all_of(l_codes))
clinic_geneve <- clean_data(clinic_extraction, FALSE, FALSE)
codes_extraction <- clinic_code[[3]] %>%
  slice(
    sapply(
      colnames(clinic_geneve), 
      function(i) str_which(clinic_code[[3]]$column_code, paste0("^", i, "$"))))
code_names <- sapply(
            seq(nrow(codes_extraction)),
            function(i) 
                    paste0(codes_extraction[i, "group_name"], " - ", codes_extraction[i, "item_name"], {res <- codes_extraction[i, "repeat_inds"]; ifelse(is.na(res), "", paste0(" - ", res))})
        )
...
res <- sapply(2:5, function(i) as.numeric(as.data.frame(clinic_geneve[, date_vars[1]] - clinic_geneve[, date_vars[i]])[, 1]))
which_min <- sapply(seq(nrow(res)), function(i) which.min(abs(res[i, ])))
which_min <- sapply(which_min, function(i) ifelse(length(i) == 0, NA, i))

t_clinic_geneve <- as.tibble(t(clinic_geneve))
colnames(t_clinic_geneve) <- clinic_geneve$C_2643_3796
t_clinic_geneve <- as.tibble(cbind(codes_extraction, t_clinic_geneve))

colnames(clinic_geneve) <- code_names

# Output
library(openxlsx)
file_path <- file.path(golem::get_golem_wd(), "clinic_extraction.xlsx")
wb <- createWorkbook()
...
get_var_names(common_vars, clinic_code[[1]])
```
...
batch <- unique(sapply(peds_files, function(i) {
    i[5]
}))
max(gsub("batch(\\d)", "\\1", batch))
peds_files <- peds_files[grepl("batch1", peds_files)]
ql <- unique(sapply(peds_files, function(i) {
    i[2]
}))
(ql <- sort(as.numeric(gsub("PedsQL(\\d+)", "\\1", ql))))
years <- unique(sapply(peds_files, function(i) {
    i[3]
}))
