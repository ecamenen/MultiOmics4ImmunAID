...
vis1 %>% filter(str_detect(item_name, "fatigue|global"))
vis1 %>% slice(detect_difficulty(., "aidai"))

## 2. Clinical manifestations
codes %>% slice(detect_difficulty(., "triggering"))
vis1 %>% filter(str_detect(group_name, "Fever"))
codes %>% slice(detect_difficulty(., "response to therapie"))
vis1 %>% slice(detect_difficulty(., "weight loss"))
codes %>% slice(detect_difficulty(., "other signs"))

## 4.1 General biology
vis1 %>%
    slice(detect_difficulty(., "CRP|SAA")) %>%
    filter(str_detect(group_name, "General biology"))
codes %>% slice(detect_difficulty(., "\tferritin"))
codes %>% slice(detect_difficulty(., "Erythrocyte"))
codes %>%
    filter(str_detect(item_name, "Date")) %>%
    filter(str_detect(group_name, "General biology"))
codes %>% slice(detect_difficulty(., "date of inclusion"))

## 5. Systemic therapies
sys <- codes %>% filter(str_detect(group_code, "SystemicTherapies"))
sys %>% filter(str_detect(item_name, "Treatment group|Date of beginning|Response|over|Treatment category|Treatment$|Every|Period"))
sys %>% slice(detect_difficulty(., "\tadministrated"))
...
codes %>% slice(detect_difficulty(., "date of the visit"))
codes %>%
    slice(detect_difficulty(., "evolution")) %>%
    filter(!str_detect(item_name, "Persistant"))

## 3.3 Biological values
vis2 <- codes %>% slice(detect_difficulty(., "second|followup"))
vis2 %>% slice(detect_difficulty(., "CRP|SAA"))
```
