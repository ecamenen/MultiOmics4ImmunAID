...
summary_cat <- function(i) {
  df <- as.data.frame(clinic_tot[, i])[, 1] %>% fct_explicit_na() %>% as.data.frame() %>% set_colnames("var")
...
# Manifestations
manifs <- c("MUSCU", "CARDIO", "GASTRO", "PULMO", "OPHTALMO", "ENT", "HAEMATO")
manifs <- c("MUCO_ACTIVE_LESIONS", paste0(manifs, "_MANIF"))
# NEURO, URO
map(manifs, function(x) immu_manif(x, df2 = clinic_tot0) %>%
    collapse_mcat() %>%
    plot_bar_mcat()) -> p
...
tree <- data.tree::FromDataFrameNetwork(edges)
