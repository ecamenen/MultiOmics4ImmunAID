...
rownames(elisa) <- elisa$sample_barcode
...
for (i in var_na) {
    clinic_tot_proc[clinic_tot_proc == i] <- NA
}
...
clinic_tot_proc <- rename_with(
  clinic_tot_proc,
  ~ str_sub(.x, 1, 20)
)
colnames(clinic_tot_proc) <- get_var_names(colnames(clinic_tot_proc), clinic_code[[1]])
clinic <- tibble(clinic_tot_proc, .name_repair = vctrs::vec_as_names)
row.names(clinic) <- clinic_tot_proc$immun_aid_identifier

clinic <- clinic_tot_proc
...
rownames(pql) <- as.data.frame(pql[, 1])[[1]]
