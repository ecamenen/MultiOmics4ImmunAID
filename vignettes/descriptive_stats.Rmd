...
clinic_proc <- best_na_percent(clinic_num, 50)

# Remove constant variables
clinic_proc <- clean_data(clinic_proc, clean_name = FALSE)
rownames(clinic_proc) <- clinic_dis$immun_aid_identifier
clinic_proc <- colnames(clinic_proc) %>% sort() %>% select(clinic_proc, .)
colnames(clinic_proc) <- str_replace(colnames(clinic_proc), "(do_you_still_have_the_results).*", "\\1") %>% str_replace("(cerebrovascular_accident_or_).*", "\\1ischemic")

clinic_proc <- clinic_proc[-37, ]


func <- function(x) names(ordinal_variables(clinic_proc, x))
for (i in 2:60) {
  print(setdiff(func(i + 1), func(i)))
}
# sapply(clinic_proc, unique) %>% sapply(length) %>% sort()

to_parse <- sapply(seq_along(clinic_proc), function(i) any(grepl(";", as.data.frame(clinic_proc[, i])[, 1]))) %>% which()
l_levels <- sapply(
  to_parse,
  function(i) {
    as.data.frame(clinic_proc[, i])[, 1] %>%
  str_split(";") %>%
    unlist() %>%
    str_trim() %>%
    unique() %>%
...
t_levels <- lapply(
  ...
  function(i) {
  	sapply(
  		na.omit(l_levels[[i]]),
  		function(j) {
  			as.data.frame(clinic_proc[, to_parse])[, i] %>%
  			str_detect(j) %>%
  			as.numeric
  		}
  	) %>%
...
n_levels <- str_replace(colnames(clinic_proc)[to_parse], "(at_least_3).*", "\\1") %>% str_replace(".*(parts_affected)", "\\1") %>%
...
clinic_proc <- clinic_proc %>% select(-to_parse) %>% cbind(t_levels)

# setdiff(names(ordinal_variables(clinic_proc, 3)), names(ordinal_variables(clinic_proc, 2)))

ordinal_vars <- names(ordinal_variables0(clinic_proc, 2))
clinic_ord <- clinic_proc[, ordinal_vars] %>% mutate_all(factor)
mutate_all(as.numeric) %>% as.matrix %>% heatmap(na.rm = FALSE, scale = "none", Rowv = NA, Colv = NA, margins = c(13, 0), col = colorRampPalette(c("blue", "white", "#cd5b45"))(100))

to_remove1 <- (diagnose_category(clinic_ord) %>% filter(!is.na(levels)) %>% filter(freq < 5))$variables %>% unique()
# to_remove2 <- (diagnose_category(select(clinic_ord, -c(to_remove))) %>% filter(is.na(levels)) %>% filter(freq < 5))$variables
...
temp <- t(perc)[-1,] %>%
    cbind(., round(. / nrow(clinic_proc), 2)) %>%
    as.data.frame() %>%
...
blocks_clinic_still  <-  list(CLINIC = select(blocks_cl, -all_of(vars))) #[[1]] <- select(blocks_clinic_still[[1]], -all_of(vars))
...
clinic_still_transformed <- blocks_clinic_still
use_data(clinic_still_transformed, overwrite = TRUE)
```

## Qualitative variables
```{r}
codes_qual <- Reduce(rbind, lapply(colnames(clinic_proc), function(i) filter(temp2, str_detect(item_name, paste0("^", i, "$")))))
codes_qual %>% group_by(group_name) %>% summarise(n = n()) %>% arrange(n)
...
clinic_proc <- mutate_all(clinic_proc, factor)
...
temp <- select(clinic_num, starts_with("treatment_group"))
l_levels0 <- unlist(temp) %>% unique() %>% as.character()
...
)[, -7] %>% set_colnames(str_replace(na.omit(l_levels0), "\\(.+\\)", ""))
temp <- temp %>% as_tibble %>% set_colnames(paste0("treatment_", colnames(temp))) %>% clean_names()
...
clinic_qual <- select(clinic_proc, -c(to_remove, to_remove2, "clinical_center", "liver_disease")) %>% cbind(data.frame(temp, immun_aid_identifier = clinic_dis$immun_aid_identifier)[-37, ])
clinic_qual <- finalise_data(clinic_qual, row_name = "immun_aid_identifier", clean_name = FALSE) %>% select(-c("immun_aid_identifier")) %>% select(sort(colnames(clinic_qual)))
