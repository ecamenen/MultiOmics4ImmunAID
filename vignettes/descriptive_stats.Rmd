...
list_prot_names <- lapply(ms, function(i) i$ProteinName) %>%
    unlist() %>%
    str_split(";") %>%
    reformat() %>%
    sort()
...
ms_proc <- t(ms_tot) %>%
    cbind(colnames(ms_tot), .) %>%
    tibble()
colnames(ms_proc[[1]]) <- c("MajorProteinIds", ms_tot$MajorProteinIds)

ms <- ms_proc[[1]][-seq(4), ] %>%
    cbind(ms_codes_tot) %>%
    tibble() %>%
    select(all_of(colnames(ms_codes_tot)), everything()) %>%
    select(-MajorProteinIds) %>%
    clean_data(FALSE) %>%
    update_columns(4, dmy)

ms_code <- ms_proc[[1]][seq(4), ] %>% tibble()

use_data(ms, overwrite = TRUE)
use_data(ms_code, overwrite = TRUE)
```
...
vars_elisa <- get_patient_id(elisa)
vars_ms <- get_patient_id(ms)
vars_prot <- list(ELISA = vars_elisa, MS = vars_ms)
vars_tot <- c(list(Clinic = vars_clin), vars_prot)
vars_tot2 <- c(vars, vars_prot)
plot_venn(vars_tot)
...
# Save
colnames(chaq_tot) <- group_codes
chaq <- clean_data(chaq_tot)
...
# Save
colnames(behcet_tot) <- group_codes
behcet <- clean_data(behcet_tot)
...
# Save
colnames(diet_tot) <- group_codes
diet <- clean_data(diet_tot)
