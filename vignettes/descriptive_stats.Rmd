...
l_codes <- c(l_codes, (codes %>% filter(str_detect(item_name, "identifier")) %>%
...
l_codes <- c(l_codes, (codes %>%
    ...
    filter(str_detect(group_name, "General biology")))$column_code)
l_codes <- c(l_codes, (codes %>% slice(detect_difficulty(., "date of inclusion")))$column_code)

## 5. Systemic therapies
sys <- codes %>% filter(str_detect(group_code, "SystemicTherapies"))
l_codes <- c(l_codes, (sys %>% filter(str_detect(item_name, "Treatment group|Date of beginning|Response|over|Treatment category|Treatment$|Every|Period")))$column_code)
l_codes <- c(l_codes, (sys %>% slice(detect_difficulty(., "dose")))$column_code)
...
l_codes <- c(l_codes, (codes %>% slice(detect_difficulty(., "date of the visit")))$column_code)
l_codes <- c(l_codes, (codes %>%
