...
clinic_transf[clinic_transf == -Inf] <- -1
...
for (i in seq_along(keys)) {
    temp <- str_replace(temp, keys[i], make_clean_names(values[i]))
}
...
clinic_transf <- finalise_data(clinic_transf, row_name = "C_2643_3796", clean_name = FALSE)
colnames(clinic_transf) <- temp
attributes(clinic_transf)$codes <- cbind(code_info, item_name = as.character(temp))
