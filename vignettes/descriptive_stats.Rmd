...
clinic_dis <- clinic %>%
    filter(
        str_detect(
            disease,
            replace_parenthesis(paste(c(diseases[9], diseases[1]), collapse = "|"))
        )
    )
levels(clinic_dis$disease)[diseases[1] == levels(clinic$disease)] <- "1"
levels(clinic_dis$disease)[diseases[9] == levels(clinic$disease)] <- "0"
clinic_dis$disease <- clinic_dis$disease %>%
    factor(., levels = unique(.))

# Batch * centers effects
# (ggplot(data=(clinic_dis %>% group_by(clinical_center) %>%   mutate(count_name_occurr = n())), aes(x=reorder(clinical_center,-count_name_occurr), fill=as.character(batch))) +
#     geom_bar(position = "stack") + labs(fill = "Batch")) %>% theme_histo(90)
...
clinic_proc <- clean_data(clinic_proc)
...
clinic_proc0 <- clinic_proc[, sort(colnames(clinic_proc))] %>%
    select(-all_of(c(var_occ, "immun_aid_identifier")))
