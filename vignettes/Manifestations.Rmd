...
par(mfrow = c(2, 4))
for (x in seq_along(diseases)) {
    per_disease_perc[, diseases[x]] %>%
        set_names(
            names(.) %>%
        ...
        plot_radar(
            ...
            title = paste0(diseases[x], ", N = ", sample_size0[x])
        )
}
# dev.off()
```
...
ids <- list.map(
    c("manif", "active_description"),
    f(x) ~{
        list.map(vars, f(y) ~ get_manif(y, x, l = list_manif))
    }
)  %>%
    unlist() %>%
    unique() %>% 
    sort() %>%
    discard(function(x) x %in% c("First episode", "Recurrent", "Continuous"))
ids <- list(
    c("Multiple", "Unique"),
...
stat_manifs <- apply(res[[3]], 2, sum) %>% sort()
manif_cat <- list.mapv(names(stat_manifs), f(x) ~ list.which(manifs, f(y) ~ str_detect(x, y) %>% any())) %>% rev()
# %>% titles[.] 
plot_bar(
    stat_manifs,
    colour = GimmeMyPlot:::palette_discrete()[manif_cat[seq(10)]], 
    sample_size = 450,
    title = "Manifestations",
    width_title = 50,
    n_max = 10
...
res1 <- as.data.frame(res[[3]]) %>% rownames_to_column("C_2643_3796") %>% mutate( C_2643_3796 = as.numeric(C_2643_3796)) %>% right_join(clinic0)
p <- list.map(
    diseases0,
    f(x) ~{
        ...
        stat_manifs <- apply(res2, 2, sum) %>% 
         ...
         manif_cat <- names(stat_manifs) %>%
             list.mapv(f(x) ~ list.which(manifs, f(y) ~ str_detect(x, y) %>% any())) %>%
             head(10)
