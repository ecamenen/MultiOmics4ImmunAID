                    ...
                    f(j, jt) ~ {
                        ...
                        if (type == 1) {
                            j <- str_remove_all(j, "[\\[\\{\\}\\]\\\"]") %>%
                                ...
                                str_split(", ") %>%
                                pluck(1) %>%
                                list.map(j, f(k) ~ if (is.na(k) | (length(k) == 1 && str_detect(k, ";"))) k else str_to_sentence(k)) %>%
                                unlist() %>%
                                paste0(collapse = " ; ")
                        }
                        if (type == 2) {
...
names(l_manif) %in% c("is_active", "active_description", "type", "non_active_description") %>%
    l_manif[.] %>%
    lapply(. %>% mutate(., `NA` = rowSums(.) %>% `==`(0) %>% as.numeric())) %>%
    list.map(
        f(i, j, k) ~ {
...
get_manif("MUSCU", "active_description") %>%
    get_occ_list() %>%
    plot_pie(colour = brewer_pal(, "Reds")(9)[c(9, 3, 6)], dec = 1, cex = 20, hsize = 1.6, label = FALSE, title = "Chronicity")
