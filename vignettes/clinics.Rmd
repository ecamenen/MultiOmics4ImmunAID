...
```{r}
temp <- read.xlsx(file.path(path, "AOSD_SOJIA_Reponse_traitements.xlsx"), startRow = 2, cols = 3:8) %>% set_rownames(pull(., 1)) %>% set_colnames(c("id", "Corticoteroid", paste0("Anti-IL", c(1, 6)), "DMARDs", "Kinase Inhibitor"))
...
emna_treat <- sapply(seq(2), function(i) as.numeric(all_ids %in% temp0[, i])) %>% data.frame() %>% set_colnames(paste0("with", c("out", ""), " treatment history")) %>% mutate(id = all_ids) %>% full_join(temp) %>% set_rownames(pull(., id)) %>% select(-id)
