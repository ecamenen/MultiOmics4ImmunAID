...
df_manifs0 <- select(clinic_tot1, temp$column_code) %>% list.map(f(i) ~ as.factor(i) %>% fct_recode(`0` = "No", `1` = "Yes", `NA` = "Unknown") %>% as.character() %>% as.numeric()) %>% list_cbind() %>%
set_colnames(vars1)

apply(df_manifs0, 2, function(x) sum(x, na.rm = TRUE)) %>% as.data.frame() %>% set_colnames(c("N")) %>% kable0()
...
still_manif <- df_manifs %>%
    ...
    arrange(ids) %>%
    select(-ids) %>%
    set_rownames(ids) %>% cbind(df_manifs0, .)
...
vars <- paste0("IMA_SYSTHERAPY_TREATMENT_", c("BEGIN_DATE", paste0("GROUP", c("", "_OTHER_NEW_VALUE")), "OVER", "RESP", paste0("STOP_", c("DATE", paste0("REASON", c("", "_OTHER_NEW_VALUE"))))))
# "Date",
var_names0 <- c("Date of beginning", "Type", "Drug name", "Discontinuation", "Response", "Date of discontinuation", paste0("Reason for discont.", c("", " (other)")))
...
vars <- c("((PRECISION)|(TREAT))", "((aINF)|(aPLAT)|(DOSE))", "FREQUENCY", "DURATION")
var_names1 <- c("Drug name", "Administrated dose", "Frequency", "Period")
list_treat <- list.map(
    treats,
    f(i, j) ~ {
        list.map(
...
list_treat <- list.map(c("Aspirin", "IVIG", "Colchicin"), f(k) ~ list.map(list_treat, f(i) ~ list.map(i, f(j) ~ if(j$Type == k) list(j$Type) %>% set_names("Drug name")) %>% compact()) %>% compact()) %>% compact() %>%
    Reduce(function(x, y) list.merge(x, y), .) %>%
    list.merge(list_treat, .)
...
```{r}
vars <- c("C_2643_3796", "C_2643_3802", "C_2733_4210", "C_2742_4210", "C_2643_3798")
var_names <- c("ID", paste("Date of the", c(paste0(c("1st", "2nd"), " visit"), "follow-up")), "Disease")
list_date <- list.map(
    vars,
    f(i, j) ~ {
        temp <- clinic_code[[1]] %>%
...
var_names2 <- c("Date of beginning", "Drug name", "Type", "Subtype", "Administrated dose", "Dose", "Frequency", "Period", "Response", "Discontinuation", "Date of discontinuation", "Reason for discont.", "Reason for discont. (other)")
list_treat1 <- Reduce(bind_rows, list_treat0) %>%
    as_tibble() %>%
    group_by(ID2) %>%
    mutate(`N previous treatment` = unique(`Pre-inclusion.Drug name`) %>% na.omit() %>% length()) %>%
    ungroup() %>%
    select(., contains(c(var_names, "N previous treatment", paste(rep(c("Pre-inclusion", "Ongoing treatment", "Post-inclusion"), each = length(var_names2)), var_names2, sep = "."))))

list_treat2 <- select(list_treat1, -ID2) %>% set_colnames(colnames(.) %>% str_remove_all("((Patient)|(((Post)|(Pre))-inclusion)|(Ongoing treatment))."))
...
list.map(list_treat00, f(i) ~ list.filter(i[["Post-inclusion"]], f(j) ~ j$Type == "Biologic")) %>% compact()

treat_types <- list.map(list_treat, f(i) ~ list.map(i, f(j) ~ j[names(j) == "Type"])) %>%
    unlist() %>%
    sort() %>%
    unique()
t_treat_types <- list.map(treat_types, f(i) ~ list.mapv(list_treat0, f(j) ~ i %in% j$`Post-inclusion.Type`) %>% as.numeric()) %>% list.cbind()
...
treat_subtypes <- list.map(list_treat, f(i) ~ list.map(i, f(j) ~ j[names(j) == "Subtype"])) %>%
    ...
    sort() %>%
    unique()
t_treat_subtypes <- list.map(treat_subtypes, f(i) ~ list.mapv(list_treat0, f(j) ~ i %in% j$`Post-inclusion.Subtype`) %>% as.numeric()) %>% list.cbind()
apply(t_treat_subtypes, 2, sum)

list.map(treat_types, f(i) ~ list.map(list_treat0, f(j) ~ {
    if (all(c("Post-inclusion.Type", "Post-inclusion.Drug name") %in% colnames(j))) {
        filter(j, `Post-inclusion.Type` == i) %>%
            pull(`Post-inclusion.Drug name`) %>%
            unique()
    } else {
        NULL
    }
...
list.search(list_treat0, "Anakinra" %in% .) %>%
    list.match("Post-inclusion") %>%
    length()
l_per_treat <- list.map(treat_types, f(i) ~ {
    l_drugs <- list.map(list_treat0, f(j) ~ if (all(c("Post-inclusion.Type", "Post-inclusion.Drug name") %in% colnames(j))) filter(j, `Post-inclusion.Type` == i) %>% pull(`Post-inclusion.Drug name`)) %>%
        unlist() %>%
        unique()
    list.map(list_treat0, f(j) ~ {
        list.mapv(l_drugs, f(k) ~ (all(c("Post-inclusion.Type", "Post-inclusion.Drug name") %in% colnames(j))) && (k %in% (filter(j, `Post-inclusion.Type` == i) %>% pull(`Post-inclusion.Drug name`))))
    } %>% as.numeric()) %>%
        list.rbind() %>%
...
l_per_bio_treat <- list.map(treat_subtypes, f(i) ~ {
    l_drugs <- list.map(list_treat0, f(j) ~ if (all(c("Post-inclusion.Subtype", "Post-inclusion.Drug name") %in% colnames(j))) filter(j, `Post-inclusion.Subtype` == i) %>% pull(`Post-inclusion.Drug name`)) %>%
        unlist() %>%
        unique()
    list.map(list_treat0, f(j) ~ {
        list.mapv(l_drugs, f(k) ~ (all(c("Post-inclusion.Subtype", "Post-inclusion.Drug name") %in% colnames(j))) && (k %in% (filter(j, `Post-inclusion.Subtype` == i) %>% pull(`Post-inclusion.Drug name`))))
    } %>% as.numeric()) %>%
        list.rbind() %>%
        set_colnames(l_drugs)
}) %>% compact()
...
list.map(treat_types, f(k) ~ list.map(list_treat, f(i) ~ list.filter(i, f(j) ~ is.list(j) && j$Type == k) %>% list.map(f(k) ~ k$`Drug name`)) %>%
    ...
    unlist() %>%
    unique()) %>% compact() -> tmp
tmp[-seq(2)] %>% get_intersection0()
tmp[-c(3:4)] %>% get_intersection0()
tmp[-c(6:7)] %>% get_intersection0()
# DMARDs:Other` "Methotrexate" "Ciclosporine"

list.map(list_treat, f(i) ~ list.map(i, f(j) ~ if(is.list(j) && !is.null(j$Subtype) && !(j$Subtype %in% c("Anti-CD20", "Anti-IL1", "Anti-IL17", "Anti-IL5", "Anti-IL6", "Anti-TNFα"))) j$Subtype %>% set_names(j$`Date of beginning`)) %>% compact()) %>% compact() %>% unlist() 
...
list.map(list_treat, f(i) ~ list.filter(i, f(j) ~ if(is.list(j) && j$Type == "Other" && j$`Drug name` %in% c("Anti-platelets aggregation", "Methotrexate", "Ciclosporine")) TRUE) %>%
             compact()) %>% compact()
list.map(list_treat, f(i) ~ list.map(i, f(j) ~ if(is.list(j) && j$Type == "Other" && j$`Drug name` %in% c("Anti-platelets aggregation", "Methotrexate", "Ciclosporine")) j$`Drug name` %>% set_names(j$`Date of beginning`)) %>% compact()) %>% compact() %>% unlist()
...
list.map(c("Response", "Reason for discont."), f(k, l) ~ list.map(treat_types, f(i) ~ list.map(list_treat0, f(j) ~ {
    if (all(c("Post-inclusion.Type", paste0("Post-inclusion.", k)) %in% colnames(j))) {
        filter(j, `Post-inclusion.Type` == i) %>%
            pull(paste0("Post-inclusion.", k)) %>%
            unique()
    } else {
        character(0)
    }
...
list.map(list_treat, f(i) ~ list.map(i, f(j) ~ if(is.list(j) && !is.null(j$`Type`) && is.null(j$`Drug name`)) TRUE) %>% compact()) %>% compact() %>% unlist() %>% names()

greens <- brewer_pal(, "YlGn")(9)[c(4, 5, 7, 9)]
cols <- c(1, get_colors()[c(seq(2), 5, 8)], rev(greens))
list.map(
  c(list(`Type of treatment` = t_treat_types), l_per_treat[-1], l_per_bio_treat, list(Biologic = t_treat_subtypes)),
  f(i, j, k) ~ {
    if (j == 8) {
        cols0 <- greens
    } else if (j == 2) {
    ...
    if(j %in% c(3:5, 8)) {
      ...
      colors <- rev(get_colors()[c(1, 3, 8, 2, 5)])
    }else {
      colors <- c("gray", cols0)
    }
    as.data.frame(i) %>%
