...
temp <- temp %>% set_colnames(str_remove_all(colnames(temp), "[Aa]u.") %>% str_replace_all("\\.", " ") %>% str_to_title()) %>% set_rownames(temp[, 1])
emna_treat <- left_join(select(clinic_tot1, 2) %>% mutate(C_2643_3796 = as.numeric(C_2643_3796)), temp, by = c("C_2643_3796" = "Id Patient")) %>% as.data.frame() %>% set_rownames(pull(., 1)) %>% select(-1) %>% set_colnames(c("Corticoteroid", paste0("Anti-IL", c(1, 6)), "DMARDs", "Kinase Inhibitor"))
