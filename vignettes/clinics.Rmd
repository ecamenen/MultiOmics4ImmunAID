...
for (i in seq(var_max)) {
    p <- p + {clinic[-out[i], ] %>% pull(var_max[i]) %>% plot_violin0()}
}
p
...
names(res) <- clinic$disease
res[res > 50]

clinic_proc <- clinic_proc[-which(res > 50), ]
...
clinic_proc_cont <- sapply(colnames(select(clinic_proc_cont, -c("creatinine", "ALAT"))), function(i) pull(clinic_proc, i) %>% get_outliers(method = "mad", c = 24))
clinic_proc_disc <- select(clinic_proc0, c(var_scores, "charlson_score", "weight_loss"))
x <- 1e-2
sapply(colnames(select(clinic_proc_disc, -c("vascular_involvement"))), function(i) pull(clinic_proc, i) %>% get_outliers(c(0 + x, 1 - x), method = "iqr")) -> clinic_proc_disc

keys <- c("vascular_involvement", "creatinine", "ALAT")
value <- c(30, 5600, 423)
