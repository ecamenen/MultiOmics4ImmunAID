...
list.map(
    aid_scores[[i]],
    f(i, j, k) ~
    plot_violin0(i, colour = color_var[j], cex = 1.2, title = k, subtitle = TRUE, wrap_title = 15, color_title = color_var[j]) +
        theme(panel.grid.minor = element_blank())
) %>%
    plot_grid(
        plotlist = .,
...
aid_scores0 <- aid_scores[[i]]
aid_scores0[aid_scores0 == 0] <- NA
...
color_var <- colorRampPalette(colors)(ncol(aid_scores0))
as.list(aid_scores0) %>%
    discard(function(x) all(is.na(x))) %>%
    list.map(
        f(i, j, k) ~
        plot_violin0(i, colour = color_var[j], cex = 1.5, size = 3.5, title = k, subtitle = TRUE, wrap_title = 15, color_title = color_var[j], pch_colour = "black", pch_alpha = 0.5) +
            theme(panel.grid.minor = element_blank())
    ) %>%
    plot_grid(
        plotlist = .,
