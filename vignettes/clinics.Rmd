...
list_treat00 <- list.map(list_treat, f(i) ~ list.class(i, f(j) ~ if (is.list(j)) {
  if (is.null(j[["Date of discontinuation"]])) {
    if (!is.null(i[["Eligible for a 2nd visit"]]) && i[["Eligible for a 2nd visit"]] == "Yes") {
      if (!is.null(i[["Date of the 2nd visit"]]) && !is.null(j[["Date of beginning"]]) && dmy(j[["Date of beginning"]]) >= dmy(i[["Date of the 2nd visit"]])) return(c("Ongoing at V1", "Ongoing at V2", "Post V2"))
  ...
  else if (!is.null(i[["Date of the 2nd visit"]])) {
...
list_treat1 <- Reduce(bind_rows, list_treat0) %>%
    as_tibble() %>%
    select(., contains(c(var_names, "N previous treatment", paste(rep(c("Pre-inclusion", paste("Ongoing at", c("V1", "V2")), "Post V2"), each = length(var_names2)), var_names2, sep = "."))))
