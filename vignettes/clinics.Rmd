...
clinic_proc_cont <- sapply(colnames(select(clinic_proc_cont, -c("creatinine", "ALAT"))), function(i) get_outliers(clinic_proc, i, method = "mad", c = 24))
clinic_proc_disc <- select(clinic_proc0, c(var_scores, "charlson_score", "weight_loss"))
x <- 1e-2
sapply(colnames(select(clinic_proc_disc, -c("vascular_involvement"))), function(i) get_outliers(clinic_proc, i, c(0 + x, 1 - x), method = "iqr")) -> clinic_proc_disc
