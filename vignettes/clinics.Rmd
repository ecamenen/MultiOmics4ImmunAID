...
clinic_tot1 <- clinic_tot0 %>%
    ...
    clean_data(FALSE, FALSE, TRUE, FALSE)
...
clinic0 <- read_batches()
clinic_tot0 <- Reduce(rbind, clinic0) %>%  clean_data(FALSE, FALSE, TRUE, FALSE)
...
list_treat0 <- list.map(list_treat, f(i) ~ list.class(i, f(j) ~ if (is.list(j)) {
...
)) %>% list.map(f(i) ~ list.map(i, f(j) ~ list.map(j, f(k) ~ unlist(k)) %>% bind_rows()) %>% list_cbind() %>% mutate(ID2 = i$Patient$ID))
...
list_treat1 <- Reduce(bind_rows, list_treat0) %>% as_tibble() %>% group_by(ID2) %>% mutate(`N previous treatment`  = unique(`Pre-inclusion.Drug name`) %>% na.omit() %>% length()) %>% ungroup() %>% select(., contains(c(var_names, "N previous treatment", paste(rep(c("Pre-inclusion", "Ongoing treatment", "Post-inclusion"), each = length(var_names2)), var_names2, sep = ".")))) %>% select(-ID2) %>% set_colnames(colnames(.) %>% str_remove_all("((Patient)|(((Post)|(Pre))-inclusion)|(Ongoing treatment))."))
