...
clinic_osteo <- filter(clinic_tot0, str_detect(C_2643_3798, "osteitis"))
...
parse_treat0 <- function(x, y = "Date", type = FALSE) {
    x %>% 
        set_colnames(rep(seq(30), ncol(.)/30)) %>%
        list.parse() %>%
        list.map(f(i) ~ {
            discard(i, is.na) %>%
                list.map({
                    ...
                    if (type == 2) {
                        ...
                        temp <- str_replace_all(temp, "Antibiot(ic|ique|herapie)", "Anti-biotic") %>%
                            ...
                            str_replace_all("Vit(amin)? d", "Vitamin D") %>%
...
list_other <- list.map(list_treat, f(i) ~ list.filter(i, f(j) ~ is.list(j) && j$Type == "Other") %>% list.map(f(k) ~ k$`Drug name`)) %>% compact() %>% unlist() %>% unique()

list.map(list_other, f(x) ~ list.map(list_treat, f(i) ~ list.filter(i, f(j) ~ is.list(j) && j$`Drug name` == x) %>% list.map(f(k) ~ k$Type)) %>% compact() %>% unlist() %>% unique())

# list_treat[["7006"]][["2"]]$Type <- "DMARDs"
```

```{r}
vars <- c("C_2643_3796", "C_2643_3802", "C_2733_4210", "C_2742_4210")
var_names <- c("ID", paste("Date of the", c(paste0(c("1st", "2nd"), " visit"), "follow-up")))
list_date <- list.map(
    vars,
    f(i, j) ~ {
        temp <- clinic_code[[1]] %>%
...
list_treat <- list.merge(list_date, list_treat)
...
list_treat0 <- list.map(list_treat00, f(i) ~ list.map(i, f(j) ~ list.map(j, f(k) ~ unlist(k)) %>% bind_rows()) %>% list_cbind() %>% mutate(ID2 = i$Patient$ID))
...
list_treat1 <- Reduce(bind_rows, list_treat0) %>% as_tibble() %>% group_by(ID2) %>% mutate(`N previous treatment`  = unique(`Pre-inclusion.Drug name`) %>% na.omit() %>% length()) %>% ungroup() %>% select(., contains(c(var_names, "N previous treatment", paste(rep(c("Pre-inclusion", "Ongoing treatment", "Post-inclusion"), each = length(var_names2)), var_names2, sep = ".")))) 
...
list.map(treat_types, f(i) ~ list.map(list_treat0, f(j) ~ {if(all(c("Post-inclusion.Type", "Post-inclusion.Drug name") %in% colnames(j))) { filter(j, `Post-inclusion.Type` == i) %>% pull(`Post-inclusion.Drug name`) %>% unique() } else {NULL}}) %>% unlist() %>% factor() %>% fct_count())

list.search(list_treat0, "Anakinra" %in% .) %>% list.match("Post-inclusion") %>% length()
l_per_treat <- list.map(treat_types, f(i) ~ {l_drugs <- list.map(list_treat0, f(j) ~ if(all(c("Post-inclusion.Type", "Post-inclusion.Drug name") %in% colnames(j))) filter(j, `Post-inclusion.Type` == i) %>% pull(`Post-inclusion.Drug name`)) %>% unlist() %>% unique() ; list.map(list_treat0, f(j) ~ {list.mapv(l_drugs, f(k) ~ (all(c("Post-inclusion.Type", "Post-inclusion.Drug name") %in% colnames(j))) && (k %in% (filter(j, `Post-inclusion.Type` == i) %>% pull(`Post-inclusion.Drug name`))) )} %>% as.numeric()) %>% list.rbind() %>% set_colnames(l_drugs)}) %>% compact()
...
list.map(c("Response", "Reason for discont."), f(k, l) ~ list.map(treat_types, f(i) ~ list.map(list_treat0, f(j) ~ {if(all(c("Post-inclusion.Type", paste0("Post-inclusion.", k)) %in% colnames(j))) { filter(j, `Post-inclusion.Type` == i) %>% pull(paste0("Post-inclusion.", k)) %>% unique() } else {character(0)}}) %>% list.map(f(i) ~ if(length(i) == 0) NA else i) %>% unlist() %>% tibble() %>% descriptive_mcat_immun(var = paste0(titles[l], " (", tolower(i), ")"), n = 38, wrap = 1000)) %>% list.rbind()) %>% list.rbind()
```
