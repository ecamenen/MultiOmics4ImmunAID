...
get_min_dates <- function(x, y) {
    ...
    # apply(clin_dates, 1, function(i) min(abs(i), na.rm = TRUE)) %>%
    #     set_names(clinic_tot1$C_2643_3796 %>% as.numeric()) %>% keep(. > 7) %>% sort() %>%
...
colnames(list_biol[[5]]) %>%
    list.map(
        f(j) ~ list.map(list_biol[c(1, 4, 5)], f(i) ~ select(i, j)) %>%
            Reduce(cbind, .) %>%
...
biol_func <- list(
    Kidney = colnames(list_biol[[1]])[c(20:22)],
    Blood = colnames(list_biol[[1]])[c(1:2, 8, 11:14, 18:19, 31)],
    WC = colnames(list_biol[[1]])[c(3:7, 9:10)],
    Liver = colnames(list_biol[[1]])[c(15, 23:30)],
    Heart = colnames(list_biol[[1]])[16:17]
)
sapply(biol_func, length)
...
color_var <- get_colors0()(length(list_biol[[1]]))
color_var <- c(color_var[1:3], colorRampPalette(color_var[4:10])(10), colorRampPalette(color_var[11:18])(7), colorRampPalette(color_var[19:26])(9), c("black", "gray"))

list_biol0 <- select(list_biol[[1]], unlist(biol_func) %>% unname())
list.map(
    list_biol0,
    f(i, j, k) ~
    plot_violin0(i, colour = color_var[j], cex = 1.2, title = k, wrap_title = 19, color_title = color_title[j]) +
...
list_biol1 <- list.map(
  list_biol, 
  f(i) ~ list.map(i, get_outliers(., method = "iqr", c = 6)) %>% compact()
)

outs <- list.map(
  list_biol, 
  f(i) ~ list.map(i, get_outliers(., method = "iqr", c = 6, replace = FALSE)) %>% compact() %>% lapply(names) %>% unlist() %>% as.numeric() %>% unique() %>% slice(clinic_tot1, .) %>% pull(C_2643_3796)
...
list.map(
  ...
  f(i, j) ~ list.map(list_biol1, .[[i]]) %>% compact() %>% as.data.frame() %>% set_colnames(paste(c("First", "Second", "Third"), "visit")) %>% plot_violin0(colour = brewer.pal(9, col[j])[c(7, 5, 3)], pch_colour = "black", pch_alpha = 0.25, size  = 3, title = i, cex = 1.5, method = "kruskal")
) %>% 
    plot_grid(
        plotlist = .,
        # align = "hv",
...
t_biol <- lapply(list_biol1[c(1, 4, 5)], function(x) list.cbind(x) %>% as.data.frame() %>% select(contains(unlist(biol_func) %>% unname()))) %>% list_cbind() %>% set_colnames(colnames(.) %>% str_replace_all("General", "V1") %>% str_replace_all("SecondVisit", "V2") %>% str_replace_all("FollowUp", "V3")) %>% set_rownames(as.numeric(clinic_tot1$C_2643_3796))
use_data(t_biol, overwrite = TRUE)

t_biol0 <- list.cbind(list_biol1[[1]]) %>% as.data.frame() 
...
t_biol0 %>% plot_corr0(method = "spearman")
dev.off()
res <- t_biol0 %>% get_corr1()
i <- res$C %>% `>`(0) %>% which(arr.ind = TRUE) %>% as.data.frame()
cols <- get_colors()[c(2, 2, 1, 1, 3)]
...
list.map(
    aid_scores[[i]],
    f(i, j, k) ~
    plot_violin0(i, colour = color_var[j], cex = 1.2, title = k, wrap_title = 15, color_title = color_var[j]) +
        theme(panel.grid.minor = element_blank())
) %>%
    plot_grid(
        plotlist = .,
...
# list.map(aid_scores0, f(x, y, z) ~ is.na(x) %>% which() %>% length() %>% rep(z, .)) %>% unlist %>% plot_bar_mcat(cex = 6, wrap = 50, df0 = aid_scores[[i]])
list.mapv(aid_scores[[i]], f(x) ~ which(x > 0) %>% length()) %>% plotHistogram(df = ., cex = 2, rows = nrow(aid_scores[[i]]))
color_var <- colorRampPalette(colors)(ncol(aid_scores0[[i]]))
as.list(aid_scores0[[i]]) %>%
    purrr::discard(function(x) all(is.na(x))) %>%
    list.map(
        f(i, j, k) ~
        plot_violin0(i, colour = color_var[j], cex = 1.5, size = 3.5, title = k, wrap_title = 15, color_title = color_var[j], pch_colour = "black", pch_alpha = 0.5, lim1 = 0, lim2 = 30) +
            theme(panel.grid.minor = element_blank())
    ) %>%
    plot_grid(
        plotlist = .,
...
list.map(
    names(aid_scores0[[1]])[c(9, 13)], 
    f(i, j) ~ list.map(aid_scores0, .[[i]]) %>% data.frame() %>% set_colnames(paste(c("First", "Second"), "visit")) %>% plot_violin0(colour = c(color_var0[j], alpha(color_var0[j],  alpha = 0.5)), pch_colour = "black", pch_alpha = 0.25, size  = 3, title = i, cex = 1.5, method = "wilcox", lim1 = 0, lim2 = 30)
) %>% 
    plot_grid(
        plotlist = .,
        align = "hv",
...
plot_cor2(select(aid_scores0[[1]], c(tmp[[1]][1], tmp[[1]][2])), color = "red", cex = 1.5)
```

# General activity
```{r}
# C_2646_3876, C_2649_3898, C_2650_3902, C_2654_3919, C_2757_4185, C_2757_4186, C_2650_3916, C_2650_3917, C_2657_3932, C_2657_3933, C_2658_3934, C_2658_3935, C_2658_3936, C_2768_4204, C_2656_3928, C_2656_3929, C_2657_3930, C_2905_4527, C_2905_4531, C_2905_4534, C_2742_4229, C_2653_3918, C_2657_3931, C_2905_4538, C_2742_4212, C_2761_4200, C_2764_4191, C_2765_4198, C_2654_3920, C_2654_3921, C_2654_3922, C_2736_3919
...
# i <- "_SecondVisit"
# Pyrexia
metadata <- clinic_code[[1]] %>% filter(str_detect(group_code, paste0("^IMA", i, "_General_pyrexia")))
df <- select(clinic_tot1, metadata$column_code) %>% set_colnames(metadata$item_name)
plot_piechart(
    pull(df, 4),
    colour = brewer.pal(9, "Reds")[c(9, 7, 5)],
    cex = 40,
    dec = 1,
    hsize = 1.5,
    title = "",
...
list.map(c(2, 3), f(i, j) ~ pull(df, i) %>% plot_violin0(title = "", colour = brewer.pal(12, "Paired")[9 - j], pch_colour = "black", pch_alpha = 0.25, size  = 3)) %>% 
    plot_grid(
        plotlist = .,
        # align = "hv",
        nrow = 1,
...
# Weigth loss
metadata <- clinic_code[[1]] %>% filter(str_detect(group_code, paste0("^IMA", i, "_General_weightLoss")))
df <- select(clinic_tot1, metadata$column_code) %>% set_colnames(metadata$item_name)
df0 <- df
df0[df0 == 0] <- NA
list.map(df0, f(i, j) ~ plot_violin0(i, title = "", colour = brewer.pal(11, "Paired")[12 - j], pch_colour = "black", pch_alpha = 0.25, size  = 3, lim1 = 0)) %>% 
    plot_grid(
        plotlist = .,
        # align = "hv",
        nrow = 1,
...
t_general <- cbind(t_general, df) %>% set_colnames(colnames(.) %>% str_remove_all(" \\(unintentional\\)") %>% to_title())
plot_cor2(pull(df0, 1), pull(df0, 2), xlab = "Weight loss", ylab = "How many months?", color = "purple")

# Mother
metadata <- clinic_code[[1]] %>% filter(str_detect(group_code, "^IMA_BiologicalParents_guSAIDs_mother"))
df <- select(clinic_tot1, metadata$column_code) %>% set_colnames(metadata$item_name)
list.map(df, f(i, j, k) ~ {
    plot_piechart(
        i,
...
pull(df, 11) %>% unique() %>% na.omit() %>% sort() %>% to_title()

# Follow up
metadata <- clinic_code[[1]] %>% filter(str_detect(group_code, "^IMA_FollowUp")) ; select(clinic_tot1, metadata$column_code) %>% set_colnames(metadata$item_name) %>% View()
df <- select(clinic_tot1, metadata$column_code) %>% set_colnames(metadata$item_name)
...
df$`Evolution of the disease` <- df$`Evolution of the disease` %>% str_remove_all(" during the last 12 months")
df0 <- select(df, -c(1, get_name_num(df)))
df0[[col]] <- df0[[col]] %>% str_replace_all("^[Nn][Oo] ?.*", "No") %>% str_replace_all("l(mphoma)", "Ly\\1")

list.map(df0, f(i, j, k) ~ {
  if (j == 1) {
    colours <- get_colors()[c(5, 1)]
    legend_name <- NULL
  } else if (j == 2) {
    colours <- c(brewer.pal(9, "Greens")[c(9, 6, 4)], "red")
    legend_name <- df$`Evolution of the disease` %>% unique() %>% .[c(4:5, 1, 3)]
  } else {
...
select(df, get_name_num(df)) %>% select(-1) %>% list.map(f(i, j, k) ~ as.numeric(i) %>% plot_violin0(title = str_trunc(k, 30), colour = get_colors0()(4)[j], pch_colour = "black", pch_alpha = 0.25, size  = 3)) %>% 
    plot_grid(
        plotlist = .,
        # align = "hv",
        nrow = 2,
...
activity <- list.map(
  c("", "_SecondVisit"), f(i) ~ {
    metadata <- clinic_code[[1]] %>% filter(str_detect(group_code, paste0("^IMA", i, "_General_deseaseActivityAssessment$")))
    vars <- metadata$item_name %>% str_remove_all("( \\(.+\\))|( of disease activity)")%>% str_to_sentence()
    select(clinic_tot1, metadata$column_code) %>% set_colnames(vars) %>% as.list()
  }
)

list.map(
    activity, 
    f(i) ~ list.map(i, f(j) ~ get_outliers(j, method = "iqr", c = 3)) %>% compact())

activity1 <- list.map(activity, list.map(., f(j) ~ j[-7]))

clinic_tot1 %>% filter(C_2643_3796 == "05005") %>% select(C_2643_3803, C_2649_3888, C_2649_3895, C_2649_3891, C_2643_3797)

list.map(
    names(activity[[1]]), 
    f(i, j) ~ list.map(activity1, .[[i]]) %>% data.frame() %>% set_colnames(paste(c("First", "Second"), "visit")) %>% plot_violin0(colour = brewer.pal(12, "Paired")[2:1 + j*2 -2], pch_colour = "black", pch_alpha = 0.25, size  = 3, title = i, cex = 1.5, method = "wilcox", lim2 = 10.5)
) %>% 
    plot_grid(
        plotlist = .,
        # align = "hv",
...
t_general <- cbind(t_general, activity[[1]]) %>% set_rownames(as.numeric(clinic_tot1$C_2643_3796))
use_data(t_general, overwrite = TRUE)


list.map(
  combn(seq(3), 2, simplify = FALSE),
  f(i, j) ~ activity[[1]][i] %>% list.cbind() %>% as.data.frame() %>% plot_cor2(color = get_colors()[j], cex = 1.3)
) %>% 
  plot_grid(
        plotlist = .,
        # align = "hv",
...
list.map(ids, f(i, j) ~ get_manif("MUCO", i) %>%
    get_occ_list(fx = function(x) str_remove_all(x, " describes.*$") %>% str_remove_all(" is.*$")) %>%
    plot_bar_mcat(collapse = TRUE, cex = 8, wrap = 20, n = 100, ratio = 5, title = var_names1[j])) -> p
plot_grid(
...
list.map(c("manif", "symptom", "sequellae"), f(z) ~ {
    list.map(c(vars, "MUCO"), f(x) ~ list.map(list_manif, f(i) ~ list.map(i[[x]][[z]], f(j) ~ names(j))) %>% compact()) %>%
        ...
        list.map(f(x) ~ list.map(manifs, f(y) ~ get_manif(y, x, z, l = list_manif)) %>%
            compact() %>%
            unlist() %>%
            unique() %>%
            sort()) %>%
...
ids <- list.map(c(vars, "MUCO"), f(x) ~ list.map(list_manif, f(i) ~ list.map(i[[x]]$manif, f(j) ~ list.map(j, f(k) ~ names(k)) %>% compact()) %>% compact()) %>% compact()) %>%
    ...
    unlist() %>%
    unique()
list.map(ids, f(x) ~ list.map(manifs, f(y) ~ get_manif0(y, x, l = list_manif)) %>%
    compact() %>%
    unlist() %>%
    unique() %>%
    sort()) %>% compact()
                ...
                list.map({
                    ...
                    if (type == 2) {
                        for (i in c("il", "cd", "tnf")) {
                            temp <- str_replace_all(temp, paste0("Anti[- ](", i, ") ?(\\d{1,2}|α)$"), paste0("Anti-", toupper(i), "\\2"))
                        }
                    } else if (type == 3) {
...
list_treat <- list.map(l_bio_other, f(x, y, z) ~ list.map(list_treat, f(i) ~ list.filter(i, f(j) ~ is.list(j) & j$Subtype == "Other" & j$`Drug name` == z)) %>%
    compact() %>%
...
list_treat00 <- list.map(list_treat, f(i) ~ list.class(i, f(j) ~ if (is.list(j)) {
    if (is.null(j[["Date of discontinuation"]])) {
        if (i[["Eligible for a 2nd visit"]] != "NA" && i[["Eligible for a 2nd visit"]] == "Yes") {
            if (i[["Date of the 2nd visit"]] != "NA" && !is.null(j[["Date of beginning"]]) && dmy(j[["Date of beginning"]]) >= dmy(i[["Date of the 2nd visit"]])) {
                return(c("Ongoing at V1", "Ongoing at V2", "Post V2"))
            } else {
        ...
        if (!is.null(j[["Date of beginning"]]) && dmy(j[["Date of beginning"]]) >= dmy(i[["Date of the 2nd visit"]])) {
            ...
            if (dmy(j[["Date of discontinuation"]]) >= dmy(i[["Date of the 2nd visit"]])) {
                if (!is.null(i[["Date of the 1st visit"]]) && dmy(j[["Date of discontinuation"]]) >= dmy(i[["Date of the 1st visit"]])) {
                    return(c("Ongoing at V1", "Ongoing at V2"))
                } else {
    ...
    } else if (!is.null(i[["Date of the 1st visit"]]) && dmy(j[["Date of discontinuation"]]) >= dmy(i[["Date of the 1st visit"]])) {
        return("Ongoing at V1")
    }
} else {
    return("Patient")
...
list.map(list_treat, f(i) ~ list.map(i, f(j) ~ if (is.list(j) && !is.null(j$Subtype) && !(j$Subtype %in% c("Anti-CD20", "Anti-IL1", "Anti-IL17", "Anti-IL5", "Anti-IL6", "Anti-TNFα"))) j$Subtype %>% set_names(j$`Date of beginning`)) %>% compact()) %>%
    compact() %>%
    unlist()
...
```{r}
temp <- read.xlsx(file.path(path, "AOSD_SOJIA_Reponse_traitements.xlsx"), startRow = 2, cols = 3:8) %>%
    set_rownames(pull(., 1)) %>%
    set_colnames(c("id", "Corticosteroid", paste0("Anti-IL", c(1, 6)), "DMARDs", "Kinase Inhibitor"))
temp0 <- read.xlsx(file.path(path, "AOSD_SOJIA_Reponse_traitements.xlsx"), startRow = 2, cols = seq(2))
all_ids <- unlist(temp0) %>%
    unique() %>%
    na.omit()
emna_treat <- sapply(seq(2), function(i) as.numeric(all_ids %in% temp0[, i])) %>%
    data.frame() %>%
    set_colnames(paste0("with", c("out", ""), " treatment history")) %>%
    mutate(id = all_ids) %>%
    full_join(temp) %>%
    arrange(id) %>%
    set_rownames(pull(., id)) %>%
    select(-id) %>%
    mutate(Naive = `without treatment history` + `with treatment history`)
emna_treat[is.na(emna_treat)] <- 0
use_data(emna_treat, overwrite = TRUE)

df <- read.xlsx(file.path(path, "AOSD_SOJIA_Reponse_traitements.xlsx"), sheet = 3, startRow = 2, cols = seq(3)) %>%
    set_rownames(pull(., 1)) %>%
    set_colnames(c("id", "Corticosteroid", "Anti-IL1"))
df2 <- read.xlsx(file.path(path, "AOSD_SOJIA_Reponse_traitements.xlsx"), sheet = 3, startRow = 2, cols = 7:8)
all_ids <- unlist(df2) %>%
    unique() %>%
    na.omit()
treat_v2 <- sapply(seq(2), function(i) as.numeric(all_ids %in% df2[, i])) %>%
    data.frame() %>%
    set_colnames(paste0("with", c("out", ""), " treatment history")) %>%
    mutate(id = all_ids) %>%
    full_join(df) %>%
