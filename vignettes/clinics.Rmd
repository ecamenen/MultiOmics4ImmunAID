...
biol_func <- list(
    ...
    Blood = select(list_biol[[1]], contains(c("Hemoglobin", "Mean red cell volume", "Platelets", "SAA", "Fibrinogen level", "Erythrocyte sedimentation rate", "Total Bilirubin", "Protidemia", "Albuminemia", "Ferritin", "Schizocytes", "Glycosylated ferritin", "LDH", "Triglycerides"))) %>% colnames(),
...
list_biol00 <- list.map(list_biol0, get_outliers(., method = "iqr", c = 8)) %>% list_cbind() %>% set_colnames(colnames(list_biol0))
list.map(list_biol0, get_outliers(., method = "iqr", c = 8, replace = FALSE) %>% names() %>% as.numeric() %>% slice(clinic_tot1, .) %>% pull(C_2643_3796)) %>% compact()
