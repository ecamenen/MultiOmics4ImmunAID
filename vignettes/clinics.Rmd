...
treatments <- immu_manif("SYSTHERAPY_TREATMENT_GROUP", df2 = clinic_tot0)
plot_bar_mcat(treatments, cex = 8, ratio = 2, title = "All therapies")
treat_type <- treatments %>%
    colnames() %>%
    str_replace_all("\\s*\\([^\\)]+\\)", "") %>%
    sort()
treat_type <- treat_type[-c(4, 7)]
...
get_colors()[2, 1, 3, 4, 5, 6]
get_colors()[2, 3, 3, 4, 5, 6]

biologic <- c("aIL1", "aIL6", "aTNFalpha")
# "aIFalpha", , "aCD20"
keys <- paste0("SYSTHERAPY_TREATMENT_", c(paste0(c("OTHER", "NSAID", "CORTICO", "BIPHO", "DMARDS", "KINASE"), "_TREAT"), "ASPI_aINF", "BIO", paste0("BIO_TREAT_", biologic)))
values <- c("Other", "NSAIDs", "Corticosteroid", "Biphosphonate", "DMARDs", "Kinase inhibitors", "Aspirin", "Biologic", biologic)
...
p <- list.map(
    ...
    f(i, j) ~ {
        ...
        if (j == 8) {
            colnames(res) <- colnames(res) %>% str_to_title()
        }
        if (length(res) > 0) {
              plot_bar_mcat(res, cex = 6, ratio = 3, title = values[j], wrap = 50)
          }
    }
) %>%
    compact() %>%
    ...
    list.reverse()
# plist <- c(p[-1][c(1, 4)], list(p2), p[-1][c(3, 5:6, 6, 8:10, 7, 2)])
plist2 <- c(p[4], list(NULL), p[5], list(NULL), p[6:9])
p2 <- immu_manif(keys[1], df2 = clinic_tot1) %>%
    plot_bar_mcat(cex = 6, ratio = 3, title = values[1], parse = TRUE, collapse = TRUE, n = 10)
plot_grid(
    plotlist = plist2,
    nrow = 4,
    ncol = 2,
    align = "h",
    rel_heights = c(1.5, 1.2, rep(1, 2))
)
