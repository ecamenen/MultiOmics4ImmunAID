...
get_min_dates <- function(x, y) {
    clin_dates <- clinic_code[[3]] %>%
        ...
        update_columns(
            colnames(.),
            function(i) (dmy(x$C_2643_3802) - dmy(i)) %>% as.numeric() %>% ifelse(. > 7, NA, .)
        )
...
get_biol0 <-function(x, y, z = clinic_tot, parse = FALSE) {
    ...
    ids <- x %>% pull(column_code) %>% str_remove_all("_\\d$") %>% unique()
...
colnames(list_biol[[1]]) <- colnames(list_biol[[1]])%>% str_replace("Hb", "Hemoglobin") %>% str_replace("Conj Bili", "Conjugated bilirubin")
biol_func <- list(
...
it <- 0
res <- res0 <- colnames(list_biol[[1]]) %>% sort() %>% rev() %>% list.map(f(x) ~ pull(list_biol[[1]], x) %>% is.na() %>% which() %>% length()) %>% unlist() %>% sort() %>% names()
for (i in biol_func) {
  it <- it + 1
  for (j in res0) {
...
list.map(list_biol[[1]], f(x, y, z) ~ is.na(x) %>% which() %>% length() %>% rep(z, .)) %>% unlist %>% plot_bar_mcat(colors = color_cols, cex = 6, wrap = 50, df0 = list_biol[[1]])
...
colors <- c("#FEE391", "#FE9929", "#CC4C02", "#B2182B", "#D6604D", "#F4A582", "#F1B6DA", "#DE77AE",
...
as.list(aid_scores0) %>% discard(function(x) all(is.na(x))) %>% 
    list.map(
        f(i, j, k) ~ 
...
# IMA_FollowUp
best_na_percent(clinic_osteo, 2) %>% colnames() %>% list.map(f(i) ~ filter(clinic_code[[3]], str_detect( column_code, paste0("^", i, "$")))) %>% list.rbind() %>% View()
```
