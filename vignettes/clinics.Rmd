...
#
# IMA_BiologicalParents_guSAIDs_mother
# IMA_SecondVisit_General_clinManifScore
# IMA_FollowUp
best_na_percent(clinic_osteo, 2) %>%
    colnames() %>%
    list.map(f(i) ~ filter(clinic_code[[3]], str_detect(column_code, paste0("^", i, "$")))) %>%
    list.rbind() %>%
    View()
```
