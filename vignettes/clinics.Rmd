...
list_treat <- list.map(list_treat, f(i) ~ list.map(i, f(j) ~ if (!is.null(j$`Administrated dose`)) list(reformat_dose(j)) %>% set_names("Administrated dose")) %>% compact()) %>%
    compact() %>%
    list.merge(list_treat, .)
...
list_treat00 <- list.map(list_treat00, f(i) ~ {
    if (!is.null(i$`Pre-inclusion`)) {
        list.map(i$`Pre-inclusion`, f(j) ~ j$`Drug name`) %>%
            ...
            list(Patient = .)
    } # else list(`N previous treatment` = NA)
}) %>%
    compact() %>%
...
var_names2 <- c("Date of beginning", "Drug name", "Type", "Subtype", "Administrated dose", "Initial dose", "Maximal dose", "Response", "Discontinuation", "Date of discontinuation", "Reason for discont.", "Reason for discont. (other)", "Reason for discont.(side effect(s))")
