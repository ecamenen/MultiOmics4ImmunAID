...
# Centers
```{r}
clinic_tot1$C_2643_3797 %>% str_remove_all("\\d{2} - ") %>% str_remove_all(" \\(.*\\)") %>% plot_bar_mcat(collapse = TRUE, wrap = 25, ratio = 1, cex = 15, colors = c("#FEE0D2", "#FCA689", "#FB6A4A", "#A50F15", "#DC2924", "#A50F15"))

clinic_tot1$C_2643_3797 %>% str_remove_all("\\d{2} - ") %>% str_remove_all(" \\(.*\\)") %>% str_remove_all("AP\\-HP [\\-–] ") %>% str_replace_all("(.*) [\\-–] .*", "\\1") %>% plot_bar_mcat(collapse = TRUE, wrap = 30, ratio = 1, cex = 12, colors = brewer.pal(9, "Reds")[-c(1, 9)])

clinic_tot1$C_2643_3798 %>% str_remove_all(" \\(.*\\)") %>% plot_bar_mcat(collapse = TRUE, wrap = 50, ratio = 10, cex = 8, colors = c(rep("gray", 14), "red", "gray"))

lapply(clinic, function(x) pull(x, C_2643_3796) %>% sort()) %>% set_names(seq(4)) %>% plot_venn1(cex = 2, color = brewer.pal(9, "Blues")[c(3, 5, 7, 9)], cex_label = 2 * 6)

# clinic0 <- lapply(clinic, function(x) { 
#     x %>%
#         filter(str_detect(
#             C_2643_3798,
#             replace_parenthesis(paste(
...
```{r}
clinic_tot1 <- clinic_tot0 %>% formate_immunaid()
# clinic_tot1 <- filter(clinic_tot0, str_detect(C_2643_3798, "osteitis")) %>% formate_immunaid()
vars0 <- clinic_code[[3]] %>%
...
df_manifs0 <- select(clinic_tot1, temp$column_code) %>%
    ...
    list_cbind() %>%
    set_colnames(vars1) %>%
    set_rownames(pull(clinic_tot1, "C_2643_3796"))
...
clinic0 <- read_immunaid()
clinic_tot0 <- Reduce(rbind, clinic0) %>% formate_immunaid()
clinic_osteo <- filter(clinic_tot0, str_detect(C_2643_3798, "osteitis")) %>% formate_immunaid()
