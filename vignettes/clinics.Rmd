...
vars <- lapply(
  clinic,
  function(i) pull(i, C_2643_3796) %>% as.numeric()
)
names(vars) <- seq(length(vars))
plot_venn(vars)
...
plot_grid(
  plotlist = list(
plot_violin0(clinic_tot1$C_2643_3803, brewer.pal(8, "Set2")[5], cex = 1.2, size = 3, pch_alpha = 0.75, coef = 1.5),
plot_violin0(clinic_tot1$C_2649_3890, brewer.pal(8, "Set2")[6], cex = 1.2, size = 3, pch_alpha = 0.75, coef = 1.5)
),
  align = "hv",
  nrow = 1,
  ncol = 2
)
mutate(clinic_tot1, C_2649_3890 = ifelse(C_2649_3890 >= 18, "Adult", "Children")) %>% pull(C_2649_3890)  %>% table()

descriptive_stats(select(clinic_tot1, age_at_inclusion_tim)) %>% View()
# Manifestations
```
