...
immu_manif("SYSTHERAPY_TREATMENT_GROUP_OTHER_NEW_VALUE", df2 = clinic_tot0) %>%
    plot_bar_mcat(collapse = TRUE, parse = TRUE, n = 1000, wrap = 1000) #, result = TRUE) %>%
    # slice(1) %>%
    # pull(2) %>%
    # str_split(", ") %>%
    # pluck(1) %>%
...
# "Date",
var_names <- c("Date", "Type", "Name", "Over", "Response", paste("Stop", c("date", paste0("reason", c("", " (other)")))))
...
list_treat <- list.map(
    ...
    f(i, j) ~ {
        ...
        parse_treat0(temp, var_names[j], type = type)
    }
) %>% Reduce(function(x, y) list.merge(x, y), .)

list.map(list_treat, f(i) ~ list.map(i, f(j) ~ j[names(j) == "Name"]) %>% compact()) %>%
    compact() %>%
    unlist() %>%
    unname() %>%
    sort() %>%
...
var_names <- c("Name", "Administrated dose", "Frequency", "Period")
list_treat <- list.map(
    treats,
    f(i, j) ~ {
        list.map(
...
# Abatacept, Canakinumab, Enate in Group
for (x in c("Name", "Type", "Over", "Response", "Stop reason", "Stop reason (other)", "Subtype")) {
    cat("\n***\n")
    list.map(list_treat0, f(i) ~ list.map(i, f(j) ~ j[names(j) == x]) %>% compact()) %>%
        compact() %>%
        unlist() %>%
        unname() %>%
        sort() %>%
