...
list.map(
    list_biol0,
    f(i, j, k) ~
    plot_violin0(i, colour = color_var[j], cex = 1.2, title = k, subtitle = TRUE, wrap_title = 19, color_title = color_title[j]) +
...
list_biol1 <- list.map(list_biol0, f(i) ~ get_outliers(i, method = "iqr", c = 6)) %>% compact()
outs <- list.map(list_biol0, f(i) ~ get_outliers(i, method = "iqr", c = 6, replace = FALSE)) %>% compact()
slice(clinic_tot1, lapply(outs, names) %>% unlist() %>% as.numeric() %>% unique()) %>% pull(C_2643_3796)
```
