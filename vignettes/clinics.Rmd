...
list.map(c(2, 3), f(i, j) ~ pull(df, i) %>% plot_violin0(title = "", colour = brewer.pal(12, "Paired")[9 - j], pch_alpha = 0.25, size  = 3, cex = 2)) %>% 
    plot_grid(
        plotlist = .,
        # align = "hv",
        nrow = 1,
...
df0 <- df
df0[df0 == 0] <- NA
list.map(df0, f(i, j) ~ plot_violin0(i, title = "", colour = brewer.pal(11, "Paired")[11 - j], pch_alpha = 0.25, size  = 4, lim1 = 0, cex = 1.7, pch_colour = "gray40", cex_sub = 12 * 1.7)) %>% 
    plot_grid(
        plotlist = .,
        # align = "hv",
        nrow = 1,
...
list.map(select(df0, -contains("unknown")), f(i, j, k) ~ {
  if (k == "Diagnosed new disease") {
    colours <- get_colors()[c(1, rep(2, 10))]
    legend_name <- NULL
  } else if (k == "Evolution of the disease") {
    colours <- c(brewer.pal(9, "Greens")[c(9, 6, 4)], "red")[c(3, 4, 1, 2)]
    legend_name <- NULL# df$`Evolution of the disease` %>% unique() %>% .[c(4:5, 1, 3)] %>% na.omit()
  } else {
...
list.map(
    names(activity[[1]]), 
    f(i, j) ~ list.map(activity1, .[[i]]) %>% data.frame() %>% set_colnames(paste(c("First", "Second"), "visit")) %>% plot_violin0(colour = brewer.pal(12, "Paired")[2:1 + j*2 -2], pch_colour = "gray40", pch_alpha = 0.25, size  = 3, title = i, cex = 1.5, method = "wilcox", lim2 = 10.5, cex_sub = 12 * 1.5)
) %>% 
    plot_grid(
        plotlist = .,
        # align = "hv",
